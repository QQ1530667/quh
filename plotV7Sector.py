...
#left I-I
ax.fill_between(q4x,q4y,p5y,color="gainsboro")
#right I-II
ax.fill_between(p2x,p2y,q2y,color="aquamarine")
#left I-II
ax.fill_between(p5x,p5y,q3y,color="aquamarine")
#text
p=int(N/2)
#region right I-I
...
#region left I-II
tx=p5x[p]
ty=1/3*(2*p5y[p]+q3y[p])
ax.text(tx,ty,"I-II",fontsize=16)
#compute roots
g=0.01
E=g**(-1/2)*c
coefs=[-1j*g,0,1j,0,0,-E]
...
ax.set_title("$ix^{3}-igx^{5}-E=0$, $g=$"+str(g)+", $E=$"+'{:.2e}'.format(Decimal(str(E))))



plt.savefig("./potential7/"+str(coefs)+".png")